...
class DatabaseManager:
    ...
    def clear_all_data(self):
        ...
        cursor.execute("DELETE FROM tbl_slalom")
        cursor.execute("DELETE FROM tbl_lap")
        cursor.execute("DELETE FROM tbl_brake")
        cursor.execute("DELETE FROM tbl_hill")

        # Reset ID counters supaya data berikutnya dimulai lagi dari 1.
        cursor.execute(
            "DELETE FROM sqlite_sequence "
            "WHERE name IN ('tbl_slalom', 'tbl_lap', 'tbl_brake', 'tbl_hill')"
        )
